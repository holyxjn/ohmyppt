...
export const CANVAS_CONSTRAINTS = [
  ...
  '- 页面样式写在各自 page 文件内，但写入片段中禁止引用系统骨架类；使用你自己定义的页面内语义 class。',
  '- 整套页面必须复用同一套背景体系、主色与字体，不要每页换一套皮肤。',
  '- 背景必须铺满画布：背景应定义在你的最外层页面容器上，不要只给局部卡片上色导致边缘露白。',
  '- 如果你手写 section[data-page-scaffold="1"] 承载背景或主布局，它应使用 h-full min-h-0 overflow-hidden，并保持与根背景同一色系。',
...
export const CONTENT_WRITING_RULES = [
  ...
  '- 片段可以是一个完整创意页面容器，也可以是 section[data-page-scaffold]；系统会自动补 section/main/content 入口和可编辑 data-block-id。',
  '- 调用写入工具前必须自检：主要容器开闭数量一致，HTML 末尾不能停在未闭合标签内。',
  '- The page-writing tool automatically wraps your content in the standard page frame. Pass only the page fragment.',
  '- 不要为了满足结构而牺牲创意排版；标题、图表、说明和视觉模块可以自由组织。',
  ...
  '- 主要可视元素建议有稳定唯一 class；可编辑文本块的 data-block-id 可由系统自动补齐。',
