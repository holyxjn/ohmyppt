...
export const PreviewIframe = forwardRef<PreviewIframeHandle, {
  html?: string
  src?: string
  title: string
  htmlPath?: string
  ...
  inspecting?: boolean
  inspectable?: boolean
  onSelectorSelected?: (selector: string, label: string, elementTag?: string, elementText?: string) => void
  onInspectExit?: () => void
  ...
  const resolvePageHtmlPath = (inputPath?: string, currentPageId?: string) => {
  ...
  const encodePathSegments = (filePath: string) =>
    filePath
      .split('/')
  ...
  const withPreviewParams = (inputUrl: string) => {
  ...
  const safeExecuteJavaScript = (webview: Electron.WebviewTag, script: string) => {
    try {
  ...
  useImperativeHandle(ref, () => ({
    patchPageContent(targetPageId: string, newHtml: string) {
      const wv = webviewRef.current
      if (!wv) return
      safeExecuteJavaScript(wv, `
  ...
  useEffect(() => {
    ...
    const handleDomReady = () => runInspectorLifecycle()
    ...
    const handleConsoleMessage = (event: Event) => {
      ...
      try {
        ...
        if (parsed.type === 'selected' && parsed.selector) {
          onSelectorSelected?.(parsed.selector, parsed.label || parsed.selector, parsed.elementTag, parsed.elementText)
          return
        }
        if (parsed.type === 'exit') {
          onInspectExit?.()
  ...
  return (
    <div
      ref={containerRef}
      className="relative h-full w-full overflow-hidden rounded-lg border border-[#d8ccb5]/80 bg-[#fbf6ec]"
    >
      {webviewSrc ? (
        <webview
